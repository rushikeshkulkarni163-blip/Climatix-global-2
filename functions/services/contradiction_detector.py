...
# ── Cross-Document Verification (Evidence Intelligence Agent spec §5) ──────────
# Compares the SAME metric as reported across multiple evidence documents (and
# the assessment's own answer) for one question, and classifies any numeric
# difference. Hard rule: never label a difference "contradictory" before
# checking whether it's explained by reporting period, boundary, unit, or
# methodology differences first — those are checked deterministically before
# the LLM is even asked to weigh in on "unexplained."

_CROSS_DOC_CLASSES = (
    "legitimate_yoy_change", "restatement", "methodology_change",
    "acquisition_divestment", "boundary_change", "reporting_error",
    "unexplained_contradiction",
)

_CROSS_DOC_SYSTEM = """You are a climate-disclosure forensic analyst reconciling the SAME metric as \
reported across multiple sources (different documents, different reporting periods, or the \
company's own assessment answer) for one institutional climate risk assessment.

Hard rule: a numeric difference between two observations of the same metric is NOT automatically a \
contradiction. Before concluding "unexplained_contradiction", you must first check whether the \
difference is fully or partially explained by:
- a legitimate year-on-year change (the reporting periods differ and the change is plausible in \
  direction/magnitude for that time gap)
- a restatement (one source explicitly says it is a restated/corrected figure)
- a methodology change (the observations state different measurement standards/methodologies)
- an acquisition or divestment changing the reporting entity's boundary
- a boundary change (e.g. Scope 1+2 vs Scope 1+2+3, owned vs operational control)
- a reporting error (one source is internally inconsistent with itself)
- only if none of the above plausibly explains it: "unexplained_contradiction"

Return valid JSON only — no markdown, no commentary."""

_CROSS_DOC_USER_TMPL = """METRIC BEING RECONCILED: {metric_name}

OBSERVATIONS OF THIS METRIC ACROSS DIFFERENT SOURCES:
{observations_json}

Classify the relationship between these observations. If there are more than 2, focus on the pair(s) \
with the largest unexplained numeric gap.

Return ONLY this JSON:
{{
  "classification": "one of: {classes}",
  "severity": "Critical|High|Medium|Low|None",
  "explanation": "2-3 sentence institutional-grade explanation citing the specific sources/values compared",
  "sources_compared": ["<source label 1>", "<source label 2>"]
}}"""


def compare_claims_across_evidence(metric_name: str, observations: list) -> dict:
    """
    Reconciles multiple observations of the SAME metric (spec §5 worked
    example: 2024 Sustainability Report Scope 1 = X vs 2025 Annual Report
    Scope 1 = Y vs Assessment Response Scope 1 = Z).

    `observations`: list of dicts, each shaped like the metric_intelligence.py
    tuple plus a `source` label, e.g.
      {"value": "18200", "unit": "tCO2e", "reporting_period": "FY2023-24",
       "boundary": "Scope 1+2", "methodology": "GHG Protocol",
       "source": "2024 Sustainability Report"}

    Returns:
      {classification, severity, explanation, sources_compared} —
      classification is one of _CROSS_DOC_CLASSES.
    Returns classification "legitimate_yoy_change"/severity "None" with a
    plain explanation if fewer than 2 observations are given (nothing to
    reconcile), rather than fabricating a comparison.
    """
    if len(observations) < 2:
        return {
            "classification": "legitimate_yoy_change",
            "severity": "None",
            "explanation": "Only one observation of this metric is available — nothing to reconcile yet.",
            "sources_compared": [o.get("source", "unknown") for o in observations],
        }

    # Deterministic pre-check: if reporting periods, boundaries, or units
    # differ across observations, surface that plainly rather than letting
    # the LLM guess — the spec's hard rule is "check period/boundary/unit/
    # methodology BEFORE flagging a contradiction."
    periods = {o.get("reporting_period") for o in observations if o.get("reporting_period")}
    boundaries = {o.get("boundary") for o in observations if o.get("boundary")}
    units = {o.get("unit") for o in observations if o.get("unit")}
    methodologies = {o.get("methodology") for o in observations if o.get("methodology")}
    ...
    try:
        ...
        resp = client.chat.completions.create(
            ...
            temperature=0.1,
            messages=[
                {"role": "system", "content": _CROSS_DOC_SYSTEM},
                {"role": "user", "content": _CROSS_DOC_USER_TMPL.format(
                    metric_name=metric_name,
                    observations_json=json.dumps(observations, indent=2, default=str),
                    classes=", ".join(_CROSS_DOC_CLASSES),
                )},
            ],
        )
        parsed = _parse_json(resp.choices[0].message.content)
        classification = parsed.get("classification")
        if classification not in _CROSS_DOC_CLASSES:
            classification = "unexplained_contradiction" if len(units) > 1 or (len(periods) <= 1 and len(boundaries) <= 1) else "legitimate_yoy_change"
        return {
            "classification": classification,
            "severity": parsed.get("severity", "Medium"),
            "explanation": parsed.get("explanation", "").strip() or (
                f"{len(observations)} observations of {metric_name} compared across "
                f"{len(periods) or 1} reporting period(s), {len(boundaries) or 1} boundary definition(s)."
            ),
            "sources_compared": parsed.get("sources_compared") or [o.get("source", "unknown") for o in observations],
        }
    except Exception:
        # LLM unavailable — fall back to the deterministic pre-check only,
        # never silently claim "no contradiction" when sources genuinely
        # differ in a way we can't reconcile without the LLM's judgment.
        if len(boundaries) > 1:
            classification, severity = "boundary_change", "Medium"
        elif len(methodologies) > 1:
            classification, severity = "methodology_change", "Medium"
        elif len(units) > 1:
            classification, severity = "unexplained_contradiction", "Medium"
        else:
            classification, severity = "unexplained_contradiction", "High"
        return {
            "classification": classification,
            "severity": severity,
            "explanation": (
                f"Automated reconciliation unavailable — deterministic check found "
                f"{len(boundaries)} distinct boundary definition(s), {len(methodologies)} "
                f"distinct methodology reference(s), {len(units)} distinct unit(s) across "
                f"{len(observations)} observations of {metric_name}. Analyst review required."
            ),
            "sources_compared": [o.get("source", "unknown") for o in observations],
        }
